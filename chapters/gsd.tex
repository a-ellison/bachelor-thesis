...
\begin{definition}[The SD-GSD game]
	Let $\lambda \in \N$ a security parameter and let $\hgen, \hdep \colon \{0, 1\}^\lambda \to \{0, 1\}^\lambda$ two functions. Let $\Pi = (\gen, \enc, \dec)$ a public-key encryption scheme with security parameter $\eta$, where $\gen$ uses $r(\eta)$ bits of randomness. We require that $\lambda \ge r(\eta)$ when choosing values for $\lambda$ and $\eta$. Define the game $\sdgsdgame{\Pi}(\lambda, \eta)$ for an adversary $\adv$:
	\begin{enumerate}[1.]
		...
		\item $\adv$ outputs a node $v \in [n]$. We call $v$ the \emph{challenge node}. A bit $b \from \{0, 1\}$ is sampled and $\adv$ is given
		      \[
			      ...
			      \end{cases},
		      \]
		      where $s \from \{0, 1\}^\lambda$. \todo{Make it clear that $r$ is not a seed of a node and thus also not a hidden seed.} $\adv$ may continue to do queries as before.
...
\begin{definition}[SD-GSD security]
	Let $\Pi, \hgen$ and $\hdep$ as above.
	The triple $(\Pi, \hgen, \hdep)$ is \emph{$(t, \epsilon, N, \delta)$-SD-GSD-secure} if for any adversary $\adv$ constructing a GSD graph of size at most $N$ and indegree at most $\delta$ and running in $t$ time we have
	\begin{align*}
...
\begin{theorem} \label{theorem:sdgsd-security}
	Let the security parameters $\lambda$ and $\eta$ be arbitrary but fixed.
	Let $\dhies$ the DHIES scheme instantiated with a group-generation algorithm $\mathcal{G}$ where $(\mathbb{G}, q, g) \from \mathcal{G}(1^\eta)$, a private-key encryption scheme $\Pi_s$ where $\kappa = \kappa(\eta)$ and a KDF $\hdh \colon \mathbb{G} \to \{0, 1\}^\kappa$. Let $\delta, N \in \N$ arbitrary with $\delta \le N$. If $\Pi_s$ is $(t, \epsilon)$-EAV-secure, the DDH problem is $(t, \epsilon)$-hard relative to $\mathcal{G}$ and $\hdh$ is modelled as a random oracle, then $\dhies$ is $(\tilde{t}, \tilde{\epsilon}, N, \delta)$-SD-GSD-secure in the ROM with
	\[
		\tilde{\epsilon} = 2 \cdot (\delta + 1) \cdot N \cdot \epsilon + \frac{2 \cdot \mdh \cdot N^2}{q} + \frac{\ms \cdot N}{2^{\lambda - 1}},
	\]
	...
	\[
		\tilde{t} = t \begin{aligned}[t]
			- & \mathcal{O}
			\begin{aligned}[t]
				\big(  \lambda & \cdot \ms \\ + & N \cdot ((\lambda + \kappa) \cdot t_{\mathrm{sample}} + \mdh \cdot t_{\mathrm{op}} + t_{\dhies.\gen})  \\ + & N^2 \cdot t_{\dhies.\enc}\big), \\
			\end{aligned}
		\end{aligned}
	\]
	...
	\begin{itemize}
		...
		\item $t_{\dhies.\gen}$: runtime of $\dhies.\gen(1^\eta)$ (which is strictly greater than the runtime of $\dhies.\gen(1^\eta, s)$ for input randomness $s$)
...
Let $x_i = \log_g(pk_{u_i})$. Each encryption $c_i$ is a tuple of the form $\langle g^{y_i}, \Pi_s.\enc_{k_i}(s_w) \rangle$ where $y_i \from [q], k_i = \hdh\left(g^{x_i \cdot y_i}\right)$. Now we can again do a case distinction on whether $\hdh$ was queried for some group element $g^{x_j \cdot y_j}$ or not:
\begin{enumerate}[(i)]
...
\begin{proof}[of Theorem~\ref{theorem:sdgsd-security}]
	...
	\[
		\pr{\qs \land \fdh} \le N \cdot \epsilon + \frac{\mdh \cdot N^2}{q}.
	\]
	...
	so we know that
	\[
		\pr{\qs} \le (\delta + 1) \cdot N \cdot \epsilon + \frac{\mdh \cdot N^2}{q} + \frac{\ms \cdot N}{2^\lambda}.
	\]
	Then by \eqref{eq:theorem-sd-gsd-security-win-cases}
	\[
		\pr{\wins} \le (\delta + 1) \cdot N \cdot \epsilon + \frac{\mdh \cdot N^2}{q} + \frac{\ms \cdot N}{2^\lambda} + \frac{1}{2},
	\] so
	\[
		\mathrm{Adv}_{\Pi}^{\mathrm{SD-GSD}}(\adv) \le 2 \cdot \left((\delta + 1) \cdot N \cdot \epsilon + \frac{\mdh \cdot N^2}{q} + \frac{\ms \cdot N}{2^\lambda}\right) = \tilde{\epsilon}.
	\]
\end{proof}
...
Recall case (\ref{qs-triggered-case-2}) in the high-level discussion of Theorem~\ref{theorem:sdgsd-security}: the adversary $\adv$ was able to learn the seed $s_w$ given the EAV secure encryptions $\Pi_s.\enc_{k_1}(s_w), \ldots, \Pi_s.\enc_{k_d}(s_w)$. We can see $\adv$ as an adversary against a security game where $\adv$ is given $d$ EAV secure encryptions $c_1 \from \Pi_s.\enc_{k_1}(m), \ldots, c_d \from \Pi_s.\enc_{k_d}(m)$ of a message $m$ with $k_i \from \Pi_s.\gen(1^\kappa)$ and must compute $m$. If we can prove that beating such a game is hard, then we can bound the probability of $\adv$ actually learning $s_w$ in this way.
...
\begin{definition}[The MIS-EAV game]
	Let $\kappa$ denote the security parameter and let $\Pi$ a private-key encryption scheme. Define the game $\mathrm{Game}_{\adv, \Pi}^{\mathrm{MIS-EAV}}(\kappa)$ for an adversary $\adv$:
	\begin{enumerate}[1.]
...
\begin{lemma} \label{lemma:eav-reduction}
	Recall the assumptions, variables and events from the statement and proof of Theorem~\ref{theorem:sdgsd-security}. In particular, assume that $\Pi_s$ is $(t, \epsilon)$-EAV-secure. Let $\adv$ an SD-GSD adversary constructing a GSD graph of size at most $N$ and indegree at most $\delta$, making at most $\ms$ queries to $\hgen$ or $\hdep$ and at most $\mdh$ queries to $\hdh$, and running in time $\tilde{t}$. Then
	\[
		\pr{\qs \land \overline{\fdh}\,} \le \delta \cdot N \cdot \epsilon + \frac{\ms \cdot N}{2^\lambda}.
	\]
\end{lemma}
...
\begin{proof}[of Lemma~\ref{lemma:eav-reduction}]
	...
	\begin{enumerate}[1.]
		...
		\item \label{eav-reduction-mis-eav-adversary-step-2} $\adv'$ samples $w \from [n], s \from \{0, 1\}^\lambda$ and gives $\delta$ and the messages $s_w, s$ to the challenger. Let $c_1, \ldots, c_\delta$ the encryptions it gets back.
		\item $\adv'$ faithfully simulates the SD-GSD game to $\adv$ with the following exception: Whenever $\adv$ makes a query of the form $\mathrm{encrypt}(u, w)$ for any $u$, $\adv'$ replies with $\langle g^x, c_i \rangle$ where $x \from [q]$ and $i$ is the index of the next ciphertext (from step \ref{eav-reduction-mis-eav-adversary-step-2}) not yet used.

		      All random oracles queries are simulated by sampling the output of the oracle u.a.r.
	...
	\begin{equation} \label{eq:lemma-eav-reduction-G2-vs-simulation}
		\pr{W_2 = w_2} = \pr{W' = w}.
	\end{equation}
	Consider how $G_2$ differs from the game simulated by $\adv'$. Both games abort at exactly the same events (verify this! \question{Ok to add such a note for the reader?}). They only differ in how $\adv'$ answers queries $\mathrm{encrypt}(u, w)$ for any $u$. In $G_2$ such a query is answered with a ciphertext $\langle g^x, c \rangle$ where $x \from [q], c \from \Pi_s.\enc_k(s_w)$ and $k = \hdh(pk_u^x)$. $\adv'$ answers such a query with $\langle g^{x'}, c' \rangle$ where $x' \from [q], c' \from \Pi_s.\enc_{k'}(s_w)$ and $k' \from \{0, 1\}^\kappa$. Now notice that as long as the game $G_2$ is ongoing, $pk_u^{x}$ is a hidden Diffie-Hellman key and $\adv$ has not queried $pk_u^{x}$ to $\hdh$. If it had, then the game would have already aborted. Therefore, from $\adv$'s view $k$ follows the same distribution as $k'$. Thus, overall the game $G_2$ and the game simulated by $\adv'$ are indistinguishable to $\adv$ and \eqref{eq:lemma-eav-reduction-G2-vs-simulation} holds.
	...
	\begin{align*}
		\begin{split}
			...
			& + N \cdot (\lambda \cdot t_{\mathrm{sample}} + t_{\dhies.\gen})  \\
...
\begin{definition}[Key-rerandomizability] \label{def:key-rerandomizability}
	Let $\kappa$ the security parameter and $\Pi$ a private-key encryption scheme. $\Pi$ is \emph{key-rerandomizable} if there exists a probabilistic polynomial-time algorithm $\operatorname{ReRan}$ achieving the following: Given $1^\kappa$ and $c \from \enc_k(m)$ for any fixed message $m$ in the message space and $k \from \gen(1^\kappa)$, the output $c' \from \operatorname{ReRan}(1^\kappa, c)$ follows the same distribution as the process of sampling $k' \from \gen(1^\kappa)$ and computing a ciphertext $\enc_{k'}(m)$.
\end{definition}
...
\begin{proof}
	Note that since the message space and thus the ciphertext space is finite, the runtime of $\operatorname{ReRan}$ is indeed bounded. Let $\adv$ an MIS-EAV adversary running in time $\tilde{t}$ and making at most $q$ queries. We construct an EAV adversary $\adv'$ that behaves as follows:
	\begin{enumerate}[1.]
		...
		\item $\adv'$ computes ciphertexts $c_2 \from \operatorname{ReRan}(c_1), \ldots, c_q \from \operatorname{ReRan}(c_1)$ (with independent runs of $\operatorname{ReRan}$).
...
\begin{corollary}
	...
	\[
		\tilde{\epsilon} = 4 \cdot N \cdot \epsilon + \frac{2 \cdot \mdh \cdot N^2}{q} + \frac{\ms \cdot N}{2^{\lambda - 1}}
	\]
...
\begin{lemma} \label{lemma:dh-reduction}
	Recall the assumptions, variables and events from the statement and proof of Theorem~\ref{theorem:sdgsd-security}. In particular, assume that the DDH problem is $(t, \epsilon)$-hard relative to $\mathcal{G}$. Let $\adv$ an SD-GSD adversary constructing a GSD graph of size at most $N$ and indegree at most $\delta$, making at most $\ms$ queries to $\hgen$ or $\hdep$ and at most $\mdh$ queries to $\hdh$, and running in time $\tilde{t}$. Then
	\[
		\pr{\qs \land \fdh} \le N \cdot \epsilon + \frac{\mdh \cdot N^2}{q}.
	\]
\end{lemma}

\paragraph{Intuition} We will bound the simpler event $\fdh$. This event tells us that there is some safe node $a$ in the GSD graph with encryption edges to nodes $u_1, \ldots, u_d$, where the query $\mathrm{encrypt}(a, u_i)$ returned the ciphertext $\langle g^{y_i}, \Pi_s.\enc_{k_i}(s_{u_i}) \rangle$ with $k_i = \hdh(g^{sk_{a} \cdot y_i})$, such that $g^{sk_a \cdot y_j}$ was the first hidden Diffie-Hellman key queried by $\adv$ for some $j$.
Moreover, at the instance $g^{sk_a \cdot y_j}$ was queried, no hidden seed had yet been queried by $\adv$, implying that $\adv$ had not queried $\hgen(s_a)$ and thus had no information about $sk_a$ (recall that $(pk_a, sk_a) = \dhies.\gen(\hgen(s_a))$).
...
\begin{proof}[of Lemma~\ref{lemma:dh-reduction}]
	...
	\begin{enumerate}[1.]
		...
		\item $\adv'$ faithfully simulates the SD-GSD game to $\adv$ with the following exception: For the $j$-th query $\mathrm{encrypt}(a, u_j)$ made by $\adv$, $\adv'$ replies with $\langle h_2 \cdot g^{r_j}, \Pi_s.\enc_{k_j}(s_{u_j}) \rangle$ where $r_j \from [q]$, $k_j \from \{0, 1\}^\kappa$. $\adv'$ also computes and stores $R_j = \left(pk_a^{r_j}\right)^{-1}$.
	...
	\begin{equation} \label{eq:lemma-dh-reduction-advantage}
		\mathrm{Adv}_{\mathcal{G}}^{\mathrm{DDH}}(\adv') \stackrel{\eqref{eq:advantage-equality}}{=} \pr{0 \from \adv' \mid b = 0} - \pr{0 \from \adv' \mid b = 1},
	\end{equation}
	...
	\begin{enumerate}[(i)]
		\item $\adv'$ sets $pk_a = h_1$ and not to the public key output by $\dhies.\gen(\hgen(s_a))$
	...
	Second, we will show that $\pr{0 \from \adv' \mid b = 1}$ is negligible. When $b = 1$ in $\mathrm{Game}_{\adv, \mathcal{G}}^{\mathrm{DDH}}$, $k$ is a uniformly random group element independent of any information given to $\adv$, in particular of $q_i \cdot R_j$ for any $i, j$. Thus for any $i, j$,
	\[
	...
	\begin{align} \label{eq:lemma-dh-reduction-advantage-lower-bound}
		\begin{split}
			\mathrm{Adv}_{\mathcal{G}}^{\mathrm{DDH}}(\adv') \ge \frac{\pr{\fdh}}{N} - \frac{\mdh \cdot N}{q}.
		\end{split}
	\end{align}
	...
	\begin{align*}
		\begin{split}
			...
			& + N \cdot ((\lambda + \kappa) \cdot t_{\mathrm{sample}} + \mdh \cdot t_{\mathrm{op}} + t_{\dhies.\gen})  \\
			& +  N^2 \cdot t_{\dhies.\enc}\big)
		\end{split}
	\end{align*}
	...
	\[
		t_{\adv'} \le t.
	\]
	So by virtue of the DDH problem being $(t, \epsilon)$-hard relative to $\mathcal{G}$
	\[
		\mathrm{Adv}_{\mathcal{G}}^{\mathrm{DDH}}(\adv') \le \epsilon
	\]
	and if we combine this with \eqref{eq:lemma-dh-reduction-advantage-lower-bound} we get
	\begin{align*}
		\frac{\pr{\fdh}}{N} - \frac{\mdh \cdot N}{q} & \le \epsilon                                     \\
		                                             & \iff                                             \\
		\pr{\fdh}                                    & \le N \cdot \epsilon + \frac{\mdh \cdot N^2}{q},
	\end{align*}
