...
The argument would of course also work without restricting the adversary to a single pair of messages. However, we make use of this restriction to provide a tighter reduction for a certain class of schemes in \cite[Section~\ref*{sec:tighter-mis-eav-security}]{full-version}.
...
The details of the proof can be found in \cite[Section~\ref*{sec:mis-eav-from-eav-proof}]{full-version}.
...
\]
Now to determine whether $k$ is the real Diffie-Hellman key, we check whether $q_i \cdot R_j = k$ for some $i, j$. This yields an advantage of about $\pr{\fdh} / N$ (and a slightly larger runtime). The details of the proof can be found in \cite[Section~\ref*{sec:dh-reduction-proof}]{full-version}.
